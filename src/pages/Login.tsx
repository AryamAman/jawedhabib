...
export default function Login() {
  ...
  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      if (!event.origin.endsWith('.run.app') && !event.origin.includes('localhost') && !event.origin.includes('jawedhabib.vercel.app')) {
        return;
      }
    ...
    <div className="max-w-md mx-auto px-4 py-32">
      ...
      >
        ...
        <div className="mt-8 text-center text-sm text-stone-500 space-y-2">
          <div>
            Don't have an account? <Link to="/signup" className="text-stone-900 border-b border-stone-900 pb-1">Sign up</Link>
          </div>
          <div>
